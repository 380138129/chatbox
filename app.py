...
# 页面配置 st.set_page_config() This must be the first Streamlit command used on an app page, and must only be set once per page.
# 多行注释都不行，只能用单行注释
...
if user_input := st.chat_input("Type your message here..."):
    ...
    for response in chain_with_history.stream({"question": user_input}, config):
      full_response += response
    #   st.chat_message("ai").markdown(full_response)
